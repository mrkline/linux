...
static int m_can_read_fifo(struct net_device *dev, u32 rxfs)
{
	...
	if (!(fifo_header.dlc & RX_BUF_FDF) && (fifo_header.id & RX_BUF_RTR)) {
		...
		err = m_can_fifo_read(cdev, fgi, M_CAN_FIFO_DATA(0), cf->data, DIV_ROUND_UP(cf->len, 4));
		if (err)
			goto out_fail;
	}
...
static netdev_tx_t m_can_tx_handler(struct m_can_classdev *cdev)
{
	...
	if (cdev->version == 30) {
		...
		dlc = FIELD_PREP(TX_BUF_MM_MASK, putidx) |
			...
			fdflags | TX_BUF_EFC;
		err = m_can_fifo_write(cdev, putidx, M_CAN_FIFO_DLC,
				 &dlc, 1);
		if (err)
			goto out_fail;
